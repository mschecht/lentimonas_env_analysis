---
title: "Global Ocean analysis of Verrucomicrobia lentimonas isolate"
...
This notebook contains select analyses for Sichert et al., 2019 for figures and data...

A new strain of Verrucomicrobia (Verrucomicrobiae Opitutales Puniceicoccaceae Lentimonas) with a novel Polysaccharide Utilization Loci (PUL) mechanism. This analysis explored these PUL genes from the Verrucomicrobia Lentimonas strain in metagenomi samples from OSD 2014 and TARA's "prokaryotic, surface" samples.

Please begin this analyses with the `Set up environment` section.
...
set WD (working directory) variable to the directory where you downloaded the repo
```{r}
# Edit inside quotes here
WD <- "where/the/repo/is"
...
# 1. Distribution of `Verrucomicrobiae Opitutales Puniceicoccaceae Lentimonas` in OSD and TARA
...
wget -c https://ndownloader.figshare.com/articles/9904793/versions/1 -O osd_tara_mitags.zip

# Unzip
unzip osd_tara_mitags.zip

# Clean up
mv osd2014_mitag_tax.tsv.gz data/raw/osd/
mv tara_mitag_tax.tsv.gz data/raw/tara/
...
```{r}
# OSD miTAG data
osd <- data.table::fread(input = "gzcat data/raw/osd/osd2014_mitag_tax.tsv.gz",
                         sep = "\t",
                         header = TRUE, 
                         nThread = 30) %>%
  as_tibble()
...
tara <- data.table::fread(input = "gzcat data/raw/tara/tara_mitag_tax.tsv.gz",
                         sep = "\t",
                         header = TRUE, 
                         nThread = 30) %>%
  as_tibble()
...
# Save for later
dir.create(path = "data/processed/")
save(osd_mitags, tara_mitags, file = "data/processed/mitags.RData")
```
...
prep_map <- function(rel_abun, phylo_lvl_name){
  ...
  row.names(rel_abun.df) <- rnames # add in row names (taxa) to dataframe
...
Save
```{r}
save(Lentimonas_rel_abun_OSD, Puniceicoccaceae_rel_abun_OSD, Opitutales_rel_abun_OSD, Verrucomicrobiae_rel_abun_OSD, contex_OSD, file = "data/processed/OSD.RData")

save(Lentimonas_rel_abun_TARA, Puniceicoccaceae_rel_abun_TARA, Opitutales_rel_abun_TARA, Verrucomicrobiae_rel_abun_TARA, contex_TARA, file = "data/processed/TARA.RData")
```
...
```{r}
p_Lentimonas_TARA_data <- Lentimonas_rel_abun_TARA %>% 
  mutate(rel_abun = (rel_abun) * 100) %>% 
  filter(rel_abun > 0) %>%
  mutate(project = "tara")


p_Lentimonas_OSD_data <- Lentimonas_rel_abun_OSD %>% 
  mutate(rel_abun = (rel_abun) * 100) %>%
  filter(rel_abun > 0) %>%
  mutate(project = "osd")
...
plot_on_world <- function(df, title){
  ...
  p <- ggplot() +
    ...
    geom_point(data = df, 
               ...
               alpha = 0.7,
               color = "black",
               aes(x=longitude, y=latitude, fill = rel_abun, shape = project)) +
...
p_Lentimonas_TARA_data %>% 
  bind_rows(p_Lentimonas_OSD_data) %>%
  arrange(rel_abun) %>% # plot highest proportion samples on top
  plot_on_world(title = "")
```
...
# Save
ggsave(filename = "reports/figures/p_Lentimonas_TARA.pdf", plot = p_Lentimonas_TARA)
ggsave(filename = "reports/figures/p_Lentimonas_OSD.pdf", plot = p_Lentimonas_OSD)
ggsave(filename = "reports/figures/osd_eu.pdf", plot = osd_eu)
```
...
# 2. Map reads from most abundant samples to genome
...
Get FTP links from ENA and make `data/raw/osd/ftp.txt`

Set working directing for analysis
```{bash}
WD="where/the/repo/is"
...
```{bash}
# OSD
for FTP in `cat data/raw/osd/ftp.txt`;
do
  wget "${FTP}" -P data/raw/osd/
done
```

## Trim adapters
```{bash}
mkdir -p data/processed/00adapterclipping

LOCATION=data/raw/osd
LOCATION2=data/resources
LOCATION3=data/processed/00adapterclipping
...
## Merge reads
```{bash}
mkdir -p  data/processed/01fastq_merged

LOCATION=data/processed/00adapterclipping
LOCATION1=data/processed/01fastq_merged

for file in `ls "${LOCATION}"/*R1.ac.fastq`; do
  fname=$(basename "${file}")
  vsearch --threads 2 \
          --fastq_maxmergelen 291 \
          --fastq_minovlen 125 \
          --fastq_mergepairs "${file}" \
          --reverse "${file/R1/R2}" \
          --fastq_ascii 33 \
          --fastqout "${LOCATION1}"/"${fname/R1.ac.fastq/merged.ac.fastq}"\
...
```{bash}
mkdir -p  data/processed/02fastq_qual_trim/
mkdir -p  data/processed/02fastq_qual_trim/merged/

LOCATION=data/processed/01fastq_merged
LOCATION1=data/processed/merged/02fastq_qual_trim
...
```{bash}
LOCATION=data/processed/00adapterclipping
LOCATION1=data/processed/02fastq_qual_trim/paired
...
Index Lentimonas genome
```{bash}
mkdir -p data/processed/03mapping

bowtie2-build data/external/CC151_full.fasta data/processed/03mapping/CC151_full
```

Inspect the index just built
```{bash}
bowtie2-inspect data/processed/03mapping/CC151_full -s
```
*Should have both the genome and plasmid read
...
```{bash}
FULL=data/external/CC151_full.fasta
OUTPUT=data/processed/03mapping/anvio
...
```{bash}
anvi-run-hmms -c data/processed/03mapping/anvio/CONTIGS.db \
              --num-threads 50
```
...
anvi-init-bam -o data/processed/03mapping/anvio/CC151_full.anvio.bam \
                 data/processed/mapping/CC151_full.bam
                
# Loop
LOCATION=data/raw/osd
ANVIO=data/processed/03mapping/anvio
MAPPING=data/processed/03mapping
...
### Merge all of the anvio profiles
```{bash}
ANVIO=data/processed/03mapping/anvio
...
```{bash}
LOCATION=data/processed/03mapping/anvio
...
```{bash}
MAP=data/processed/03mapping
GENOME=data/external/CC151_full.fasta
BIN=src/tools/gen_contig_cov_per_bam_table.py
OUTPUT=data/processed/03mapping/cov-per-contig
...
OUTPUT=data/processed/03mapping/cov-per-gene
MAP=data/processed/03mapping
...
```{bash}
OUTPUT=data/processed/03mapping/cov-per-gene
BIN=src/tools/get_orf_coverage_from_bam-coAssembly.awk

mawk -f "${BIN}" "${OUTPUT}"/CC151_full_OSD113.coverage.hist > "${OUTPUT}"/CC151_full_OSD113.orf-coverage.txt
```

Check out coverage results of lentimonas PUL genes from OSD113 metagenome
```{r}
# Import gene coverage
cov_per_gene <- data.table::fread(input = "data/processed/03mapping/cov-per-gene/CC151_full_OSD113.orf-coverage.txt", 
                                      sep = "\t", 
                                      header = FALSE) %>%
  as_tibble() %>% 
...
# Import annotation
annotations <- read_csv("data/external/PULS.csv", col_names = TRUE)
...
OSD133 had the most coverage out of all OSD metagenomes, lets assemble it to see if we can detect and homologoues from the Lentomonas PUL

## Assemble OSD113 samples with [PLASS](https://github.com/soedinglab/plass)

### Assemble OSD113 with PLASS
```{bash}
# Try one
...
# run loop                
READS=data/processed/02fastq_qual_trim/paired
OUTDIR=data/processed/04assemblies/plass
...
## Search assembly vs GENOME
### Search Plass assembly vs GENOME
```{bash}
GENOME=data/external/CC151.faa
METAGENOME=data/processed/04assemblies/plass
LOCATION=data/processed/06search
...
awk -f src/external/evalue_06_filter.awk <(sort -k1,1  "${LOCATION}"/"${fname}"/"${fname}"_resultDB.m8) >  "${LOCATION}"/"${fname}"/"${fname}"_resultDB_09.m8
done
```
...
Check out results
```{r}
verr_vs_OSD_sensitive <- data.table::fread(input = "data/processed/06search/resultDB_sensitive_e09.m8", 
                                      sep = "\t", 
                                      header = FALSE) %>%
  as_tibble() %>%
...
verr_vs_OSD <- data.table::fread(input = "data/processed/06search/resultDB_e09.m8", 
                                      sep = "\t", 
                                      header = FALSE) %>%
  as_tibble() %>%
  rename(query_id = V1, target_id = V2, seqIdentity = V3, evalue = V11) %>%
