...
## Assemble OSD113 samples with [PLASS](https://github.com/soedinglab/plass) and [metaspades](https://www.ncbi.nlm.nih.gov/pubmed/28298430)
...
METAGENOME=data/processed/assemblies_plass/assembly.fas
LOCATION=data/processed/search
...
Check out results
```{r}
verr_vs_OSD_sensitive <- data.table::fread(input = "data/processed/search/resultDB_sensitive_e09.m8", 
                                      sep = "\t", 
                                      header = FALSE) %>%
  as_tibble() %>%
...
verr_vs_OSD <- data.table::fread(input = "data/processed/search/resultDB_e09.m8", 
                                      sep = "\t", 
                                      header = FALSE) %>%
  as_tibble() %>%
  rename(query_id = V1, target_id = V2, seqIdentity = V3, evalue = V11) %>%
